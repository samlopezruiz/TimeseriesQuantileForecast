...
def hmm_filename(hmm_cfg):
    return ('regime_' + '_'.join(hmm_cfg['vars'])).replace("^", "")
...
def subsets_and_test_filename(cfg):
    data_cfg, split_cfg = cfg['data_cfg'], cfg['split_cfg']
    d_text = dwn_smple_text(data_cfg)
    return 'split_' + data_cfg['inst'] + '_' + data_cfg['subfolder'] + d_text + '_' + data_cfg['trim_data_from'] + '_to_' + \
...
def subset_filename(data_cfg):
    d_text = dwn_smple_text(data_cfg)
    return 'subset_' + data_cfg['inst'] + '_' + data_cfg['subfolder'] + d_text + '_' + data_cfg['trim_data_from'] + '_to_' + \
